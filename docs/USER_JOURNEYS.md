...
fixture data. It does not call a provider, write to SQLite, or contact the
installed service. It is designed to teach and exercise the real contract:
...
The installed CLI, API, and operator UI share the actual Python comparison
engine and SQLite system of record:

1. Start `assay serve --db .assay/runs.sqlite` and require both `/healthz` and
   `/readyz` before sending work.
2. Ingest each already-scored run with its scorer contract. ASSAY reconstructs
   the dataset content hash and scorer hash before accepting the immutable run.
3. Select two compatible persisted run IDs in `/app/`, or submit them through
   `POST /api/v1/comparisons`.
4. The service executes the paired comparison and durably stores the parameters,
   version pins, verdict, interval, and every paired specimen.
5. Inspect the same evidence through the UI, API, `assay evidence`, or an
   exported JSON file.
6. Feed `assay gate` into CI: exit `0` means `PASSED`, `1` means `BLOCKED`, and
   `2` means `UNDETERMINED` or invalid input.
7. On a contract mismatch, correct the run pair and retry. The service returns a
   structured hard error and creates no comparison evidence.
8. Back up the SQLite system of record according to
   [`OPERATIONS.md`](OPERATIONS.md); do not expose the unauthenticated service
   directly to an untrusted network.

This path evaluates supplied predictions and scores. Provider execution remains
outside the implementation, so `assay run` does not pretend to launch an agent
or model.
